...
const capFn = `function sheetCaption(t){\n  if(t.caption)return t.caption;\n  const n=String(t.name||'');\n  if(/MV-1/i.test(n))return 'this is MV-1. if you only open one file of mine, make it this.';\n  if(t.site==='X')return 'face stays off. the rest of me does not.';\n  if(t.site==='OF_FREE')return /cam/i.test(n)?'just logged off. free gets the look. paid got what i did after.':'paid already has the rest. this is the only thing this page gets.';\n  if(t.site==='OF_PAID')return /cam/i.test(n)?'just logged off. still warm and it is someone\\'s fault.':'this one is staying on paid. not cropping it.';\n  if(t.site==='DMS')return /tip/i.test(n)?'{name}, thank you for {tip} tonight. if that was fun, the paid page is where the rest lives.':'hey — new scene going out tonight. $10, no pressure either way.';\n  if(t.site==='PORNHUB')return 'no face. short cut of the longer file.';\n  if(t.site==='MANYVIDS')return 'title says the scene, preview on, tags filled.';\n  if(t.site==='CHATURBATE')return 'back after a break — be gentle or don\\'t';\n  return '';\n}\nfunction sheetHow(t){\n  if(t.site==='DMS')return 'Same $10 offer. Broadcast, not just-for-you. Attach from the app. NO tracking link in the message.';\n  if(/MV-1/i.test(String(t.name||'')))return 'Post the FULL MV-1 VIDEO on OnlyFans Paid as a wall post. Do not use a photo. If LUXX has no video on this card, use the ManyVids file from her phone. Caption only. NO tracking link.';\n  if(t.site==='OF_FREE')return 'Post the privacy-safe PHOTO from the card. NO link in the caption.';\n  if(t.site==='OF_PAID')return 'Post the privacy-safe file on paid. Copy the caption. NO tracking link.';\n  if(t.site==='X')return 'Post the PHOTO on the card. One line. Do NOT paste a link. Website field already has https://onlyfans.com/luxx4free/c6 . If you see her face in the frame, skip and pick another.';\n  if(t.site==='PORNHUB')return 'This is a VIDEO job. Paste this as the LAST LINE of the video description, nothing else on that line: https://onlyfans.com/luxx4free/c10 . Do not put that link in Official Website. Official Website stays https://onlyfans.com/luxx4free/c7 .';\n  if(t.site==='MANYVIDS')return 'VIDEO listing. Use the full scene file, not a random still. Native listing. NO OnlyFans tracking link on ManyVids.';\n  if(t.site==='CHATURBATE')return 'Room title from the card. Tip prices are hers. NO OnlyFans link required in the title.';\n  return '';\n}\n`;
...
domain = domain.replace(
  ...
  "export function taskWantsMedia(task){\n  const n=String(task&&task.name||'');\n  const site=String(task&&task.site||'');\n  if(site==='MANYVIDS'||site==='PORNHUB')return 'VIDEO';\n  if(/MV-1|manyvids|\\bvideo\\b|\\bclip\\b|hero/i.test(n))return 'VIDEO';\n  return 'IMAGE';\n}\nexport function bindFileForTask(state,task,assets=[]){\n  const want=taskWantsMedia(task);\n  let pool=bindableAssets(state,task.site,assets).filter(a=>a&&a.media_type===want);\n  if(!pool.length&&(task.site==='X'||task.site==='PORNHUB')){\n    pool=(assets||[]).filter(a=>a&&a.media_type===want&&a.authorization_status==='AUTHORIZED'&&a.availability!=='QUARANTINED'&&a.availability!=='UNAVAILABLE'&&(a.variants||[]).some(v=>v&&(v.privacy_safe_export===true||v.metadata_stripped===true))&&!usedOnSite(a,task.site,state));\n  }\n  if(!pool.length)return null;"
);
domain = domain.replace(
  'if(t.asset_id){\n      bound=assets.find(a=>a.asset_id===t.asset_id)||null;',
  'if(t.asset_id){\n      bound=assets.find(a=>a.asset_id===t.asset_id)||null;\n      if(bound&&bound.media_type!==taskWantsMedia(t)){bound=null;t.asset_id=null;}'
);
domain = domain.replace(

...
export const VERSION='1.11.1-invoke';
...
export function seedState(){return {
  schemaVersion:VERSION,buildName:'LUXX v1.11.1 — Invoke',parent:{name:'LUXX Simplified Revenue Pilot v1.2.1',sha256:'e9d271b68415600f0ec19dc10829dd73f30c4c7e7bcdce25c39279c930d5dc1c'},
  rev:0,
...
export const TASK_SITES=['CHATURBATE','OF_FREE','OF_PAID','MANYVIDS','PORNHUB','X','DMS'];
export function taskList(state){return (state.tasks||[]).slice().sort((a,b)=>String(a.date||'').localeCompare(String(b.date||''))||String(a.created_at||'').localeCompare(String(b.created_at||'')));}
export function createTask(state,payload){
  ...
  if(!TASK_SITES.includes(site))throw new Error(`Task site must be one of ${TASK_SITES.join(', ')}.`);
  const date=String(payload?.date||'').trim();
  if(!/^\d{4}-\d{2}-\d{2}$/.test(date))throw new Error('Task needs a date as YYYY-MM-DD.');
  const target=payload?.target==null||payload.target===''?null:Number(payload.target);
  if(target!==null&&(!Number.isFinite(target)||target<0))throw new Error('Task target must be zero or more.');
  // The asset store lives outside domain state, so the id is recorded as given.
  const asset_id=payload?.asset_id?cleanId(payload.asset_id):null;
  const task={task_id:uid('TASK'),date,name,site,target,asset_id,
    price:payload?.price?String(payload.price).slice(0,12):null,
    earns:payload?.earns===true,
    needs_file:payload?.needs_file===true,
    done:false,completed_at:null,money:null,count:null,note:String(payload?.note||'').slice(0,400),
    source:String(payload?.source||'HOLD_SHEET'),created_at:new Date().toISOString()};
  state.tasks=[...(state.tasks||[]),task];
  appendAudit(state,'TASK',task.task_id,'TASK_CREATE',{date,name,site,target});
  return task;
}
export function completeTask(state,payload){
  ...
  // Zero is a real answer. Only null/undefined means "not recorded", so a job that
  // earned nothing is distinguishable from a job nobody entered a number for.
  const money=payload?.money==null||payload.money===''?null:Number(payload.money);
  const count=payload?.count==null||payload.count===''?null:Number(payload.count);
  if(money!==null&&(!Number.isFinite(money)||money<0))throw new Error('Enter money as zero or more.');
  if(count!==null&&(!Number.isFinite(count)||count<0))throw new Error('Enter the count as zero or more.');
  t.done=payload?.done===false?false:true;
  t.completed_at=t.done?new Date().toISOString():null;
  t.money=money;t.count=count;
  if(payload?.note!=null)t.note=String(payload.note).slice(0,400);
  appendAudit(state,'TASK',t.task_id,'TASK_COMPLETE',{done:t.done,money,count});
  return t;
}
export function tasksForDate(state,date){return taskList(state).filter(t=>t.date===date);}
// The plan in config is the source of truth. If a stored job does not match it, the job
// is stale and gets the current wording, price and file flag before it is ever shown.
export function reconcileHoldSheet(state,doc){
  if(!doc||!Array.isArray(doc.days))return 0;
  let n=0;
  for(const day of doc.days){
    const existing=(state.tasks||[]).filter(t=>t.date===day.date&&t.source==='HOLD_SHEET');
    const steps=day.steps||[];
    if(existing.length!==steps.length)continue;
    for(let i=0;i<steps.length;i++){
      const t=existing[i],step=steps[i];
      const want={name:String(step.name||''),note:String(step.note||'').slice(0,1200),
        price:step.price?String(step.price).slice(0,12):null,
        earns:step.earns===true,needs_file:step.needs_file===true,
        target:step.target==null?null:Number(step.target)};
      if(t.name===want.name&&t.note===want.note&&t.price===want.price&&
         t.earns===want.earns&&t.needs_file===want.needs_file&&t.target===want.target)continue;
      Object.assign(t,want);n++;   // recorded work is never touched
    }
  }
  return n;
}
export function importHoldSheet(state,doc){
  // The month is data, not code. One document per creator, imported once.
  if(!doc||!Array.isArray(doc.days))throw new Error('Hold sheet needs a days array.');
  const made=[];let refreshed=0;
  for(const day of doc.days){
    const date=String(day?.date||'').trim();
    if(!/^\d{4}-\d{2}-\d{2}$/.test(date))throw new Error(`Hold sheet day needs a date, got ${date||'(none)'}.`);
    const existing=(state.tasks||[]).filter(t=>t.date===date&&t.source==='HOLD_SHEET');
    const steps=day.steps||[];
    if(existing.length===steps.length){
      // Same day, same jobs: refresh the wording and flags, never the recorded work.
      // Money, counts and done stay exactly as she entered them.
      for(let i=0;i<steps.length;i++){
        const t=existing[i],step=steps[i];
        t.name=String(step.name||t.name);
        t.note=String(step.note||'').slice(0,1200);
        t.target=step.target==null?null:Number(step.target);
        t.price=step.price?String(step.price).slice(0,12):null;
        t.earns=step.earns===true;
        t.needs_file=step.needs_file===true;
        refreshed++;
      }
      continue;
    }
    ...
    for(const step of steps)
      made.push(createTask(state,{...step,date,source:'HOLD_SHEET'}));
  }
  appendAudit(state,'HOLD_SHEET',String(doc.name||'SHEET').slice(0,60),'HOLD_SHEET_IMPORT',{days:doc.days.length,tasks_created:made.length,tasks_refreshed:refreshed});
  made.refreshed=refreshed;
  return made;
}
export function monthBoard(state,{from,to}={}){
  const inRange=t=>(!from||t.date>=from)&&(!to||t.date<=to);
  const rows=taskList(state).filter(inRange);
  const byWeek={};
  for(const t of rows){
    const d=new Date(t.date+'T00:00:00Z'), day=(d.getUTCDay()+6)%7;
    const monday=new Date(d);monday.setUTCDate(d.getUTCDate()-day);
    const wk=monday.toISOString().slice(0,10);
    const w=byWeek[wk]||(byWeek[wk]={week_of:wk,assigned:0,done:0,money:0,money_recorded:false,by_site:{}});
    w.assigned++; if(t.done)w.done++;
    if(t.money!=null){w.money+=t.money;w.money_recorded=true;}
    const b=w.by_site[t.site]||(w.by_site[t.site]={assigned:0,done:0,money:0,count:0,count_recorded:false});
    b.assigned++; if(t.done)b.done++;
    if(t.money!=null)b.money+=t.money;
    if(t.count!=null){b.count+=t.count;b.count_recorded=true;}
  }
  const weeks=Object.values(byWeek).sort((a,b)=>a.week_of.localeCompare(b.week_of));
  const total={assigned:rows.length,done:rows.filter(t=>t.done).length,
    money:rows.reduce((n,t)=>n+(t.money||0),0),
    money_recorded:rows.some(t=>t.money!=null)};
  // Money with nothing recorded is NOT zero earnings; it is an unmeasured month.
  return {weeks,total,unrecorded:rows.filter(t=>t.done&&t.money==null&&t.count==null).length};
}


// ---------------------------------------------------------------------------
// FILE BINDING
// The creator does not pick the file. The app binds one before she opens the day,
// using rules that are checkable rather than taste: never used on that site,
// face-safe if the destination demands it, least recently used first.
// Visual score is deliberately NOT consulted. A sharp photo is not a selling photo,
// and letting brightness decide would attribute results to a cause that did not act.
// ---------------------------------------------------------------------------
export const SITE_TO_PLATFORM={OF_FREE:'OnlyFans Free',OF_PAID:'OnlyFans Paid',
  MANYVIDS:'ManyVids',PORNHUB:'Pornhub',X:'X',CHATURBATE:'Chaturbate',DMS:'OnlyFans Free'};
export function usedOnSite(asset,site){
  const platform=SITE_TO_PLATFORM[site];
  if(!platform)return false;
  return (asset.used_on||[]).includes(platform);
}
export function bindableAssets(state,site,assets=[]){
  const platform=SITE_TO_PLATFORM[site];
  const restricted=faceRestrictedPlatforms().includes(platform);
  return assets.filter(a=>{
    if(a.authorization_status!=='AUTHORIZED')return false;
    if(a.availability==='QUARANTINED'||a.availability==='UNAVAILABLE')return false;
    if(a.status==='PROCESSING_FAILED')return false;
    // Every job needs a privacy-safe export: the original never leaves the vault.
    // Two flags mean the same thing depending on which pipeline produced the variant.
    // Video derivatives carry privacy_safe_export; image derivatives carry
    // metadata_stripped, which is what LIBRARY shows as PRIVACY SHIELD DERIVATIVE READY.
    // Checking only one of them made every photo in the library unbindable.
    if(!(a.variants||[]).some(v=>v&&(v.privacy_safe_export===true||v.metadata_stripped===true)))return false;
    // X and Pornhub are NO FACE. Fail closed: no confirmation means not eligible.
    if(restricted&&!faceSafeConfirmed(a,platform))return false;
    if(usedOnSite(a,site))return false;
    return true;
  });
}
export function bindFileForTask(state,task,assets=[]){
  ...
  // Least recently used first, then never-used, then oldest. Deterministic, so the
  // same day binds the same file every time it is generated.
  const rank=a=>[Number(a.use_count||0),String(a.last_used_at||''),String(a.created_at||''),String(a.asset_id)];
...
export function bindDay(state,date,assets=[]){
  const taken=new Set();
  const out=[];
  for(const t of tasksForDate(state,date)){
    let bound=null;
    if(t.asset_id){
      bound=assets.find(a=>a.asset_id===t.asset_id)||null;
    }else if(t.needs_file){
      // One file per day per job. A day never shows the same photo twice.
      const pool=bindableAssets(state,t.site,assets).filter(a=>!taken.has(a.asset_id));
      bound=bindFileForTask(state,{...t},pool);
    }
    if(bound)taken.add(bound.asset_id);
    out.push({...t,
      bound_asset_id:bound?bound.asset_id:null,
      bound_reason:bound?'AUTO_BOUND_LEAST_RECENTLY_USED':(t.asset_id?'ASSIGNED':'NO_FILE_NEEDED'),
      no_file_available:!bound&&!!t.needs_file});
  }
  return out;
}
...
export function applyCommand(state,assets,op,payload={}){switch(op){
  ...
  case 'TODAY_TASKS':{
      ...
      const refreshed=payload?.sheet?reconcileHoldSheet(state,payload.sheet):0;
      return {date,refreshed,tasks:bindDay(state,date,assets||[]),
        assets_seen:(assets||[]).length,
        assigned:tasksForDate(state,date).length};
    }
    case 'TASK_CREATE':return createTask(state,payload);
    case 'TASK_COMPLETE':return completeTask(state,payload);
    case 'HOLD_SHEET_IMPORT':return importHoldSheet(state,payload);

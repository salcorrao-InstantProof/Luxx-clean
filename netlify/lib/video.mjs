...
async function analyze(fp,meta,ffmpeg){let sceneTimes=[],bad=[],silence=[];try{const r=await run(ffmpeg,['-hide_banner','-i',fp,'-vf',"scale=320:-2,select='gt(scene,0.28)',showinfo",'-an','-f','null','-']);for(const m of r.err.matchAll(/pts_time:([0-9.]+)/g))sceneTimes.push(Number(m[1]));}catch{}
try{const r=await run(ffmpeg,['-hide_banner','-i',fp,'-vf','scale=320:-2,blackdetect=d=1:pix_th=0.10,freezedetect=noise=0.003:d=2','-an','-f','null','-']);bad=[...parseRanges(r.err,'black'),...parseRanges(r.err,'freeze')];}catch{}
if(meta.has_audio)try{const r=await run(ffmpeg,['-hide_banner','-i',fp,'-af','silencedetect=noise=-45dB:d=1.2','-vn','-f','null','-']);silence=parseRanges(r.err,'silence');}catch{}
return {sceneTimes,bad,silence};}
...
async function renderClip(input,out,start,duration,ffmpeg){await run(ffmpeg,['-y','-ss',String(start),'-i',input,'-t',String(duration),'-map','0:v:0','-map','0:a?','-map_metadata','-1','-map_chapters','-1','-c:v','libx264','-preset','ultrafast','-crf','23','-maxrate','5M','-bufsize','10M','-c:a','aac','-b:a','128k','-movflags','+faststart',out]);}
async function makeHls(input,dir,ffmpeg){await fs.mkdir(dir,{recursive:true});await run(ffmpeg,['-y','-i',input,'-map','0:v:0','-map','0:a?','-map_metadata','-1','-map_chapters','-1','-vf',"scale=w='if(gt(ih,480),-2,iw)':h='if(gt(ih,480),480,ih)'",'-c:v','libx264','-preset','ultrafast','-b:v','900k','-maxrate','1200k','-bufsize','2400k','-c:a','aac','-b:a','96k','-hls_time','4','-hls_playlist_type','vod','-hls_flags','independent_segments','-hls_segment_filename',path.join(dir,'seg-%05d.ts'),path.join(dir,'index.m3u8')]);}
...
async function variantRecord(asset,tmp,input,variantId,label,start,duration,ffmpeg){const mp4=path.join(tmp,`${variantId}.mp4`),hlsDir=path.join(tmp,`${variantId}-hls`),poster=path.join(tmp,`${variantId}-poster.jpg`);trace(`${variantId} render start`);await renderClip(input,mp4,start,duration,ffmpeg);trace(`${variantId} render done; poster start`);await run(ffmpeg,['-y','-ss','0.5','-i',mp4,'-map_metadata','-1','-frames:v','1','-vf',"scale='min(640,iw)':-2",'-q:v','4',poster]);trace(`${variantId} poster done; thumb store start`);const media=await store(STORES.media),pb=await fs.readFile(poster),thumbnail_key=`thumb/${asset.asset_id}/${variantId}.jpg`;await media.set(thumbnail_key,toArrayBuffer(pb),{metadata:{contentType:'image/jpeg',assetId:asset.asset_id,variantId}});trace(`${variantId} thumb stored; hls start`);await makeHls(mp4,hlsDir,ffmpeg);trace(`${variantId} hls render done; hls store start`);const hls_segments=await storeHls(asset.asset_id,variantId,hlsDir);trace(`${variantId} hls stored; mp4 store start`);const privacy=await verifyPrivacySanitized(mp4,{mediaType:'VIDEO'});if(!privacy.metadata_stripped_verified)throw new Error(`Privacy verification failed for ${variantId}`);const download_parts=await storeMp4Parts(asset.asset_id,variantId,mp4),sha256=await sha256File(mp4),stat=await fs.stat(mp4);trace(`${variantId} mp4 stored`);return {variant_id:variantId,asset_id:asset.asset_id,master_asset_id:asset.asset_id,variant_type:label,start_seconds:start,duration_seconds:duration,start_timestamp:String(start),end_timestamp:String(start+duration),final_runtime:duration,thumbnail_frame_timestamp_or_reference:'0.5s',thumbnail_key,title_direction:'',description_direction:'',cta:'',tracking_route_id:null,price_if_applicable:null,price_creator_approved:false,native_listing_measurement_capability:false,platform:null,recipe:{trim_start:start,trim_duration:duration,video_codec:'h264',audio_codec:'aac'},rendered:true,rendered_at:new Date().toISOString(),sha256,size:stat.size,hls_url:`/api/hls?asset=${asset.asset_id}&variant=${variantId}`,hls_segments,download_parts,output_name:`${asset.asset_id}_${variantId}.mp4`,privacy_safe_export:true,metadata_stripped:true,privacy_verification:privacy,evidence_label:'TECHNICAL HEURISTIC — not a proven revenue-best segment'};}
export async function processVideoJob(job_id){const jobs=await store(STORES.jobs),assets=await store(STORES.assets);const job=await jobs.get(`jobs/${job_id}.json`,{type:'json'});if(!job)throw new Error('Job not found');const asset=await assets.get(`assets/${job.asset_id}.json`,{type:'json'});if(!asset||asset.media_type!=='VIDEO')throw new Error('Video asset not found');const tmp=await fs.mkdtemp(path.join(os.tmpdir(),'luxx-'));try{asset.status='PROCESSING';asset.processing_started_at=new Date().toISOString();asset.processing_error=null;await assets.setJSON(`assets/${asset.asset_id}.json`,asset);await setJob(job_id,{status:'RUNNING',progress:5});trace('assemble start');const {ffmpeg,ffprobe}=await paths(),input=await assemble(asset,tmp);trace('assemble done');asset.master_sha256=await sha256File(input);await setJob(job_id,{progress:15});trace('probe start');const meta=await probe(input,ffprobe);trace('probe done');const privacyScan=await inspectPrivacyFile(input,{mediaType:'VIDEO',ffprobePath:ffprobe});asset.privacy={...privacyScan,status:'PROCESSING_SAFE_DERIVATIVES',safe_export_variant_id:null,metadata_stripping_verified:false,updated_at:new Date().toISOString()};await assets.setJSON(`assets/${asset.asset_id}.json`,asset);await setJob(job_id,{progress:25,probe:meta,privacy:{embedded_geolocation_detected:privacyScan.embedded_geolocation_detected,sensitive_metadata_classes:privacyScan.sensitive_metadata_classes}});if(!asset.thumbnail_key){try{const poster=path.join(tmp,'MASTER-poster.jpg'),at=Math.max(0,Math.min(meta.duration*.1,Math.max(0,meta.duration-0.5)));await run(ffmpeg,['-y','-ss',String(at),'-i',input,'-map_metadata','-1','-frames:v','1','-vf',"scale='min(640,iw)':-2",'-q:v','4',poster]);const media=await store(STORES.media),pb=await fs.readFile(poster),k=`thumb/${asset.asset_id}/MASTER.jpg`;await media.set(k,toArrayBuffer(pb),{metadata:{contentType:'image/jpeg',assetId:asset.asset_id,variantId:'MASTER'}});asset.thumbnail_key=k;asset.thumbnail_frame_timestamp_or_reference=`${at.toFixed(2)}s`;await assets.setJSON(`assets/${asset.asset_id}.json`,asset);}catch{}}trace('analyze start');const analysis=await analyze(input,meta,ffmpeg);trace('analyze done');const plan=choosePlan(meta.duration,analysis);asset.analysis={duration_seconds:meta.duration,width:meta.width,height:meta.height,scene_changes:analysis.sceneTimes.length,bad_ranges:analysis.bad,silence_ranges:analysis.silence,plan,method:'FFmpeg technical heuristic: scene activity with black/freeze/silence penalties; no semantic desire/attractiveness claim'};await setJob(job_id,{progress:40,plan});
...
for(let i=0;i<targets.length;i++){
  ...
  trace(`${t.id} start`);
  const v=await variantRecord(asset,tmp,input,t.id,t.label,t.start,t.duration,ffmpeg);

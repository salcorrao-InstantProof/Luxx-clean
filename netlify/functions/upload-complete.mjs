...
export async function completeUpload(body){const asset_id=cleanId(body?.asset_id),name=String(body?.name||'').slice(0,240),type=String(body?.type||'application/octet-stream').slice(0,120),size=Number(body?.size),parts=Number(body?.parts),media_type=body?.media_type==='VIDEO'?'VIDEO':'IMAGE';if(!asset_id||!name||!Number.isFinite(size)||size<1||!Number.isInteger(parts)||parts<1)return {error:'Invalid upload manifest'};const expected=Math.ceil(size/CHUNK_SIZE);if(parts!==expected)return {error:`Expected ${expected} chunks, got ${parts}`};const media=await store(STORES.media),hash=crypto.createHash('sha256');
  ...
  const ends=await Promise.all([0,parts-1].map(i=>
    media.getWithMetadata(`original/${asset_id}/part-${String(i).padStart(6,'0')}`,{type:'arrayBuffer'})));
  for(const e of ends){
  ...
  if(tailActual!==tailExpected)
    return {error:`Last chunk is ${tailActual} bytes, expected ${tailExpected}. The upload is incomplete.`};
  ...
  const tk=`thumb/${asset_id}/thumb.jpg`,thumbnail_key=await media.getMetadata(tk)?tk:null;
  ...
  if(media_type==='VIDEO'){asset.job_id=`JOB-${crypto.randomUUID()}`;const jobs=await store(STORES.jobs);await jobs.setJSON(`jobs/${asset.job_id}.json`,{job_id:asset.job_id,asset_id,kind:'AUTO_VIDEO',status:'QUEUED',created_at:new Date().toISOString(),progress:0);}
  await assets.setJSON(`assets/${asset_id}.json`,asset);return {asset};}
export default async req=>{if(!verifySession(req))return error('Unauthorized',401);if(req.method!=='POST')return error('Method not allowed',405);const r=await completeUpload(await readJson(req));if(r.error)return error(r.error);return json({ok:true,...r});};
